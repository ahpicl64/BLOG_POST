...
- **지속적인 모니터링 및 알림**: CloudWatch, Grafana 등을 활용하여 주요 성능 지표(CPU, Memory, TPS, Latency)에 대한 대시보드를 구축하고, 임계치 초과 시 알림을 받는 시스템을 구성하여 장애를 사전에 예방.- [K6 부하 테스트 기반 백엔드 성능 병목 분석 및 점진적 최적화](#k6-부하-테스트-기반-백엔드-성능-병목-분석-및-점진적-최적화)
  - [배경 및 문제정의](#배경-및-문제정의)
  - [접근방식](#접근방식)
    - [1단계: 애플리케이션 레벨 분석 및 최적화 (JPA N+1 문제)](#1단계-애플리케이션-레벨-분석-및-최적화-jpa-n1-문제)
    - [2단계: 데이터베이스 레벨 분석 및 최적화 (비효율적인 쿼리 실행)](#2단계-데이터베이스-레벨-분석-및-최적화-비효율적인-쿼리-실행)
  - [실제 구현](#실제-구현)
    - [1. N+1 문제 해결 (Fetch Join)](#1-n1-문제-해결-fetch-join)
    - [2. 커버링 인덱스 생성 (SQL)](#2-커버링-인덱스-생성-sql)
    - [3. JPQL 프로젝션 구현](#3-jpql-프로젝션-구현)
  - [후속 고려사항](#후속-고려사항)
