...
4. **Spring Boot 버전 호환성 문제** - AWS 라이브러리와 충돌
5. **테스트 실행 실패** - AWS Secrets Manager 접근 시도
6. **프로파일 환경 분리 필요** - 로컬/개발/운영 환경 혼재

### Phase 3: 운영 환경 문제 (6/28 저녁~6/29)
7. **ALB Target Group Unhealthy** - 헬스체크 실패
8. **네트워크 바인딩 문제** - IPv4/IPv6 충돌
9. **DevTools 자동 재시작** - 프로덕션 환경 불안정
10. **dotenv 라이브러리 호환성** - 환경변수 로딩 실패
